...
class GenericImageDataset(torch.utils.data.Dataset):
    ...
    def imread_rgb_torch(Path, Size=None, interp=cv2.INTER_NEAREST): # Use only for loading RGB images
        ...
        Image = utils.np2torch(ImageCV) # Range: 0-255
    ...
    def applyMask(NOX, Thresh):
        ...
        MaskProb = utils.torch2np(torch.squeeze(F.sigmoid(Mask)))
        Masked = np.uint8((MaskProb > Thresh) * 255)
        MaskedNOCS = utils.torch2np(torch.squeeze(NOCS))
    ...
    def loadData(self):
        ...
        DatasetDir = os.path.join(utils.expandTilde(self.DataDir), os.path.splitext(self.FileName)[0])
        if os.path.exists(DatasetDir) == False:
            DataPath = os.path.join(utils.expandTilde(self.DataDir), self.FileName)
            if os.path.exists(DataPath) == False:
                if self.isDownload:
                    print('[ INFO ]: Downloading', DataPath)
                    utils.downloadFile(self.DataURL, DataPath)
            ...
            with zipfile.ZipFile(DataPath, 'r') as File2Unzip:
                ...
                File2Unzip.extractall(utils.expandTilde(self.DataDir))
    ...
    def convertData(RGB, Targets, isMaskNOX=False):
        TargetsTup = Targets
        Color00 = utils.torch2np(RGB[0:3].squeeze()).squeeze() * 255
        ...
        for T in TargetsTup:
            ...
            if nChannels == 4:
                if isMaskNOX:
                    ...
                    OutTupRGB = OutTupRGB + (utils.torch2np(T[0:3]).squeeze(),)
                OutTupMask = OutTupMask + (utils.torch2np(T[3]).squeeze(),)
            else:
                if isMaskNOX:
                    ...
                    OutTupRGB = OutTupRGB + (utils.torch2np(T[0:3]).squeeze(), utils.torch2np(T[4:7]).squeeze())
                OutTupMask = OutTupMask + (utils.torch2np(T[3]).squeeze(), utils.torch2np(T[7]).squeeze())
